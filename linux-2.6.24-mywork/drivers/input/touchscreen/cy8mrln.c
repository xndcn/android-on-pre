...
#define TSC_MAX_ABS_X   SCREEN_WIDTH
#define TSC_MAX_ABS_Y   SCREEN_HEIGHT
#define TSC_MAX_ABS_Z	DEFAULT_PRESSURE
...
typedef struct tsc_drv_data {
	...
	int             pen_state;
	int             noreferences;
	u16         	references[H_FIELDS * V_FIELDS];	
} tsc_drv_data_t;
...
static void cy8mrln_palmpre_update_references(u16 references[H_FIELDS * V_FIELDS], u16 field[H_FIELDS * V_FIELDS])
{
	int x, y;
...
static void cy8mrln_palmpre_interpolate(struct tsc_drv_data* dev, u16 field[H_FIELDS * V_FIELDS], int x, int y, struct ts_sample *out)
{
	float fx, fy;
	int tmpxy, tmpx1, tmpx3, tmpy1, tmpy3;
	int posx = dev->sensor_delta_x * x + dev->sensor_offset_x;
	int posy = dev->sensor_delta_y * y + dev->sensor_offset_y;

	tmpxy = field[field_nr(x, y)];

	if (x == (H_FIELDS - 1)) {
		tmpx3 = dev->pressure - tmpxy;
	} else {
		tmpx3 = field[field_nr(x, y) - 1];
	}
	if (x == 0)
		tmpx1 = dev->pressure - tmpxy;
	else
		tmpx1 = field[field_nr(x, y) + 1];

	if (y == (V_FIELDS - 1)) {
		tmpy3 = dev->pressure - tmpxy;
	} else {
		tmpy3 = field[field_nr(x, y) + H_FIELDS];
	}
	if (y == 0)
		tmpy1 = dev->pressure - tmpxy;
	else
	...
	//out->x = posx + fx * dev->sensor_delta_x;
	//out->y = posy + fy * dev->sensor_delta_y;
	
	out->x = posx + ((tmpx3 - tmpx1) * dev->sensor_delta_x) * 2 / (tmpxy * 3);
	out->y = posy + ((tmpy3 - tmpy1) * dev->sensor_delta_y) * 2 / (tmpxy * 3);
}
...
cy8mrln_work_handler(struct work_struct *work)
{
	...
	if (rxbuf) {	
	    memcpy(&input, rxbuf->ptr, rx_len);
	    if (dev->noreferences) {	
	        dev->noreferences = 0;
	        memcpy(&dev->references, input.field, H_FIELDS * V_FIELDS * sizeof(u16));	    
	    } else {
	        cy8mrln_palmpre_update_references(dev->references, input.field);
	        max_x = 0;
		    max_y = 0;
		    max_value = 0;
		    for (y = 0; y < V_FIELDS; y ++) {
		    	for (x = 0; x < H_FIELDS; x++) {
		    		tmp_value = input.field[field_nr(x, y)];
			...
			if (max_value > dev->noise) {
	    	    cy8mrln_palmpre_interpolate(dev, input.field, max_x, max_y, &samp);	
	    	    samp.pressure = max_value;
	    	    input_report_abs( dev->indev, ABS_X, samp.x );
                input_report_abs( dev->indev, ABS_Y, samp.y );
                input_report_abs( dev->indev, ABS_PRESSURE, samp.pressure);
                if (!dev->pen_state) {
                    dev->pen_state = 1;
                    input_report_key( dev->indev, BTN_TOUCH, 1 );
                    printk("cy8mrln press: x-%d, y-%d\n", samp.x, samp.y);
                }
...
static int __init
cy8mrln_init_input(struct tsc_drv_data *dev)
{
    int    rc;
...
static int cy8mrln_palmpre_set_scanrate(struct tsc_drv_data *dev, int rate)
{
    int rc = 0;
	int new_hz, old_hz;
	long timeout = SCAN_TIMEOUT_JIFFY;
...
static int cy8mrln_palmpre_set_verbose(struct tsc_drv_data *dev, int v)
{	
	dev->verbose = v;
	return 0;
}

static int cy8mrln_palmpre_set_sleepmode(struct tsc_drv_data *dev, int mode)
{
    int rc = 0;
	int sleep_mode = mode;	
	...
	if (dev->verbose) {
		printk("%s: SET_SLEEPMODE: %d\n",
			DRIVER, dev->sleep_mode);
	}
Done:	 
	return rc;
}

static int cy8mrln_palmpre_set_wot_scanrate(struct tsc_drv_data *dev, int rate)
{
    int rc = 0;
	int regval = rate;
...
static int cy8mrln_palmpre_set_wot_threshold(struct tsc_drv_data *dev, int v)
{
    int rc = 0;	
	int newsensval = v;
...
static int cy8mrln_palmpre_set_timestamp_mode(struct tsc_drv_data *dev, int v)
{
    int timestamp = v ? 1 : 0;
	dev->timestamp = timestamp;

	if (dev->verbose) {
		printk("%s: SET_TIMESTAMP_MODE: %d\n",
			DRIVER, dev->timestamp);
	}	
	return 0;
}

static int cy8mrln_palmpre_set_gesture_height (struct tsc_drv_data *dev, int h)
{
	dev->gesture_height = h;
	return 0;
}

static int cy8mrln_palmpre_set_noise (struct tsc_drv_data *dev, int n)
{
	dev->noise = n;
	return 0;
}

static int cy8mrln_palmpre_set_pressure (struct tsc_drv_data *dev, int p)
{
	dev->pressure = p;
	return 0;
}

static int cy8mrln_palmpre_set_sensor_offset_x (struct tsc_drv_data *dev, int n)
{	
	dev->sensor_offset_x = n;
	return 0;
}

static int cy8mrln_palmpre_set_sensor_offset_y (struct tsc_drv_data *dev, int n)
{	
	dev->sensor_offset_y = n;
	return 0;
}

static int cy8mrln_palmpre_set_sensor_delta_x (struct tsc_drv_data *dev, int n)
{
	dev->sensor_delta_x = n;
	return 0;
}

static int cy8mrln_palmpre_set_sensor_delta_y (struct tsc_drv_data *dev, int n)
{
	dev->sensor_delta_y = n;
	return 0;
}
...
cy8mrln_probe(struct spi_device *spi)
{
	...
	dev->noreferences = 1;	
	dev->pen_state = 0;
	cy8mrln_palmpre_set_verbose(dev, DEFAULT_VERBOSE);
	cy8mrln_palmpre_set_scanrate(dev, DEFAULT_SCANRATE);
	cy8mrln_palmpre_set_timestamp_mode(dev, DEFAULT_TIMESTAMP_MODE);
	cy8mrln_palmpre_set_sleepmode(dev, DEFAULT_SLEEPMODE);
	cy8mrln_palmpre_set_wot_scanrate(dev, DEFAULT_WOT_SCANRATE);
	cy8mrln_palmpre_set_wot_threshold(dev, DEFAULT_WOT_THRESHOLD);
	cy8mrln_palmpre_set_gesture_height(dev, DEFAULT_GESTURE_HEIGHT);
	cy8mrln_palmpre_set_noise(dev, DEFAULT_NOISE);
	cy8mrln_palmpre_set_pressure(dev, DEFAULT_PRESSURE);
	cy8mrln_palmpre_set_sensor_offset_x (dev, DEFAULT_SENSOR_OFFSET_X);
	cy8mrln_palmpre_set_sensor_offset_y (dev, DEFAULT_SENSOR_OFFSET_Y);
	cy8mrln_palmpre_set_sensor_delta_x (dev, DEFAULT_SENSOR_DELTA_X);
	cy8mrln_palmpre_set_sensor_delta_y (dev, DEFAULT_SENSOR_DELTA_Y);
...
static ssize_t palm_virtual_keys_show(struct kobject *kobj, struct attribute *attr,
			char *buf)
{		
	int size = 46;
	char *rom = "0x01:158:0:490:150:40:0x01:139:160:490:150:40";
	memcpy(buf, rom, size);
	return size;
}

static int cy8mrln_sysfs_create(void)
{
	int ret = 0;
	struct sysfs_ops sysops = {
		.show = palm_virtual_keys_show,
	};
	...
	struct kobject board_properties = {
		.k_name = "board_properties",
		.kset = &kset,
	};		
	struct attribute virtualkeys = {		
		.name = "virtualkeys.cy8mrln",
		.owner = NULL,
		.mode = 0444,
	};	
	ret = sysfs_create_dir(&board_properties);
	if (ret)
		return ret;
	ret = sysfs_create_file(&board_properties, &virtualkeys);
	if (ret)
		return ret;
	return 0;
}
...
{	
	int ret;
	//ret = cy8mrln_sysfs_create();
	//if (ret) {
	//	printk(KERN_ERR "Failed to create cy8mrln sysfs\n");
	//}
	PDBG("%s:\n", __FUNCTION__ );
